...
class Drone:
    ...
    def clean_data(self):
        ...
        if self.parte == 'a' or self.parte == 'b':
            self.data.to_csv('DataCleanResults/Scenario_'+self.scenario+'/[1]_DRONE_data_cleaned_' + self.scenario + self.parte +'.csv')
        else:
            self.data.to_csv(
                'DataCleanResults/Scenario_' + self.scenario + '/[1]_DRONE_data_cleaned_' + self.scenario + '.csv')
