...
def merge_all_sensors(scenari):
    ...
    for scenario in scenari:
        ...
        alvira = pd.read_csv('2-AlviraClustering/[2]_ALVIRA_data_predict_' + scenario + '.csv',
                             delimiter=',')
        ...
        venus = pd.read_csv('1-DataCleaned/Scenario_'+scenario+'/[1]_VENUS_data_cleaned_' + scenario + '.csv',
                            delimiter=',')
        if scenario[:1] == '1':
            drone = pd.read_csv('1-DataCleaned/Scenario_'+scenario+'/[1]_DRONE_data_cleaned_' + scenario + '.csv',
                                delimiter=',')
        if scenario[:1] == '2':
            drone_a = pd.read_csv('1-DataCleaned/Scenario_'+scenario+'/[1]_DRONE_data_cleaned_' + scenario + 'a.csv',
                                delimiter=',')
            drone_b = pd.read_csv(
                '1-DataCleaned/Scenario_'+scenario+'/[1]_DRONE_data_cleaned_' + scenario + 'b.csv',
                delimiter=',')
        ...
        merge_to_drone.to_csv('3-DataMerged/[3]_MERGE_scenario_' + scenario + '.csv',
                     float_format='%.6f')
