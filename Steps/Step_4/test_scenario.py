...
from Steps.Step_0.DataAnalisysAndClean import start_analisys
from Steps.Step_0.Outliers import clean_arcus_outliers
from Steps.Step_1.clustering import k_means
from Utility.GPS import GPSVis
import pickle
import matplotlib.pyplot as plt

from Utility.plot_speed_alt import print_speed, print_altitude
...
def merge_all_sensor_test(alvira, arcus, diana, venus, scenario):
    ...
    merge.to_csv('MergeResults/Test/[3-TEST]_MERGE_scenario_' + scenario + '.csv',
                 float_format='%.6f')
...
def arcus_meno_alvira_test(alvira, arcus, scenario):
    ...
    arcus_meno_alvira.to_csv('DataCleanResults/Scenario_' + scenario + '/[1]_ARCUS_data_cleaned_' + scenario + '.csv')
...
def predict_lat_test(scn):
    scenario = pd.read_csv('MergeResults/Test/[3-TEST]_MERGE_scenario_' + scn + '_test.csv')
    ...
    with open('Models/model_latitude.pickle', 'rb') as file:
...
def predict_lon_test(scn):
    ...
    with open('Models/model_longitude.pickle', 'rb') as file:
...
def print_lat_long_prediction(scenario, numero, lettera):
    vis = GPSVis(
        data_path='MapsResults/PredictionDroneFlight/Test/LatitudeLongitudeData/LatitudeLongitude_test_prediction_scenario_' + numero + lettera + '.csv',
        map_path='Utility/map1.png',  # Path to map downloaded from the OSM.
        points=(51.5246, 5.8361, 51.5103, 5.8752))  # Two coordinates of the map (upper left, lower right)

    vis.create_image(color=(0, 0, 255), width=3)  # Set the color and the width of the GNSS tracks.
    vis.plot_map(output='save', save_as='MapsResults/PredictionDroneFlight/Test/Figure/Maps_prediction_scenario_' + scenario)
...
def print_lat_lon_test_predette(lat_pred, lon_pred, numero, lettera):
    ...
    test_lat.to_csv(
        'MapsResults/PredictionDroneFlight/Test/LatitudeLongitudeData/LatitudeLongitude_test_prediction_scenario_' + numero + lettera + '.csv',
...
def predict_speed_test(scn):
    ...
    with open('Models/model_speed.pickle', 'rb') as file:
...
def predict_altitude_test(scn):
    ...
    with open('Models/model_altitude.pickle', 'rb') as file:
...
def start_prediction(numero, lettera):
    ...
    lat_pred = predict_lat_test(numero + lettera)
    lon_pred = predict_lon_test(numero + lettera)

    lat_pred.to_csv('PredictionResults/Test/Latitude_Longitude/latitude_pred_scenario_' + numero + lettera + '.csv', index=False,
                      float_format='%.6f')
    lon_pred.to_csv('PredictionResults/Test/Latitude_Longitude/longitude_pred_scenario_' + numero + lettera + '.csv',
                    index=False,
                    float_format='%.6f')
    ...
    speed_pred = predict_speed_test(numero + lettera)
    speed_pred.to_csv('PredictionResults/Test/Speed/speed_data_result_' + numero + lettera + '.csv',
                    index=False)
    # predici altitude

    altitude_pred = predict_altitude_test(numero + lettera)
    altitude_pred.to_csv('PredictionResults/Test/Altitude/altitude_data_result_' + numero + lettera + '.csv',
                      index=False)
    ...
    print_speed([numero+lettera], 'Test')
    print_altitude([numero+lettera], 'Test')
    print('End')
...
def data_clean(numero, lettera):
    ...
    start_analisys(alvira,'Test')

    alvira.clean_data()

    if lettera != 'd':
        clustering_alvira_test(alvira,numero)
        alvira = pd.read_csv('AlviraClusteringResults/Test/Alvira_data_predict_' + numero + lettera + '_test.csv')
    else:
    ...
    start_analisys(arcus, 'Test')

    arcus.clean_data()

    arcus = pd.read_csv(
        'DataCleanResults/Scenario_' + numero + lettera + '_test/[1]_ARCUS_data_cleaned_' + numero + lettera + '_test.csv')
    ...
    start_analisys(diana, 'Test')

    diana.clean_data()

    diana = pd.read_csv(
        'DataCleanResults/Scenario_' + numero + lettera + '_test/[1]_DIANA_data_cleaned_' + numero + lettera + '_test.csv')
    ...
    start_analisys(venus, 'Test')

    venus.clean_data()

    venus = pd.read_csv(
        'DataCleanResults/Scenario_' + numero + lettera + '_test/[1]_VENUS_data_cleaned_' + numero + lettera + '_test.csv')
...
def clustering_alvira_test(alvira,scenario):
    ...
    data = pd.read_csv('AlviraClusteringResults/Test/Alvira_data_predict_' + alvira.scenario + '.csv')
...
def print_on_map(data, scenario):
    ...
    data.to_csv('AlviraClusteringResults/Test/LatitudeLongitudeValues/AlviraClustering_LatLong_' + scenario + '.csv', header=False,
                index=False)

    vis = GPSVis(data_path='AlviraClusteringResults/Test/LatitudeLongitudeValues/AlviraClustering_LatLong_' + scenario + '.csv',
                 map_path='Utility/map1.png',  # Path to map downloaded from the OSM.
                 points=(
                     51.5246, 5.8361, 51.5103, 5.8752))  # Two coordinates of the map (upper left, lower right)

    vis.create_image(color=(0, 0, 255), width=3)  # Set the color and the width of the GNSS tracks.
    vis.plot_map(output='save', save_as='AlviraClusteringResults/Test/MapResults/AlviraClustering_Map_' + scenario)
