...
def clustering_alvira(scenari, calc_k, k, clustering_alg='Kmeans'):
    for scenario in scenari:
        data = pd.read_csv('1-DataCleaned/Scenario_' + scenario + '/[1]_ALVIRA_data_cleaned_' + scenario + '.csv',
                           delimiter=',')
        ...
        if clustering_alg == 'Kmeans':
            print('K-means')
            k_means(alvira, k)
            data = pd.read_csv('2-AlviraClustering/[2]_ALVIRA_data_predict_' + scenario + '.csv',
                               delimiter=',')

            '''Questa funziona stampa su mappa i risultati ottenuti con il K-Means'''
        ...
        if clustering_alg == 'Aglomerative':
...
def clustering_arcus(scenari):
    for scenario in scenari:
        data = pd.read_csv('1-DataCleaned/Scenario_' + scenario + '/[1]_ARCUS_data_cleaned_' + scenario + '.csv',
                           delimiter=',')
...
def print_on_map(data, scenario, sensore):
    ...
    data.to_csv('Steps/Step_1/' + sensore + 'MapsClustering/' + sensore + '_lat_long_' + scenario + '.csv',
                header=False, index=False)

    vis = GPSVis(data_path='Steps/Step_1/' + sensore + 'MapsClustering/' + sensore + '_lat_long_' + scenario + '.csv',
    ...
    vis.plot_map(output='save', save_as='Steps/Step_1/AlviraMapsClustering/'+sensore+'_clustering_flight_' + scenario)
